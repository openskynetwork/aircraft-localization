...
def RMSE_2D(gt_df,test_df):#gt_df,test_df
    """
    Calculates 2D RMSE distance between two dataframes
    ...
    gt_df.index=gt_df['id']
    test_df.index=test_df['id'].values.astype(int)    
...
def addxyz(df,order=[0,1,2]): # order : (lla)
    """
    extract ECEF xyz from lat lon alt and append to df
    
    Inputs:
    df : dataFrame
    order : columns of lla in dataFrame
    """
    tmp_xyz=[]
